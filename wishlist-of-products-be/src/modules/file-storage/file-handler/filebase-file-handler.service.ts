...
export class FirebaseFileHandlerService extends FileHandlerService{
    constructor(private readonly admin: FirebaseAdminService, private readonly fileHelper: FileHelperService){super(fileHelper)}
    
    async save(file: Express.Multer.File, folder?:string): Promise<SaveFileResult> {
        const path = this.fileHelper.createFilePath(this.fileHelper.generateUniqueFilename(file.originalname),folder);
